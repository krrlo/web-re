...
function fetchCallback(result) {
	...
	let rawdata = result.data;   //목록이 들어있음 //[{}] // rawdata = [{},{}]
	console.log(rawdata); //원래의  데이터 


	let sidoary = []; // 중복된 값이 제거된 애가 들어가있음 

	for (let val of rawdata) {
		if (sidoary.indexOf(val.sido) == -1) {
	...
	sidoary.forEach(obj => {       //17개 시도 옵션 추가 
	...
	function changeCallback (e) {    //이벤트가 발생하면 함수를 실행하세요 
		...
		let filterAry = rawdata.filter(center => {
			return center.sido == search;             //내가 선택한 값과 일치하는 center.sido만 출력하게 
		})   //조건에 만족하는 애들만 배열에 추가해줌 

		console.log(filterAry)   //조건에 만족하는애만 나온 결과   //광주광역시 애들만 나옴   [{…}, {…}, {…}, {…}, {…}, {…}
